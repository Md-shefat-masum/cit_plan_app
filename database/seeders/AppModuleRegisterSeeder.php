...
class AppModuleRegisterSeeder extends Seeder
{
    ...
    {
        $this->command->info('Starting management routes registration...');
        ...
        try {
            ...
            if (empty($managementGroups)) {
                $this->command->warn('No management groups found in routes file.');
                return;
            }

            $this->command->info('Found ' . count($managementGroups) . ' management group(s).');
            ...
            foreach ($managementGroups as $groupName => $groupData) {
                $appModules[] = [
                    ...
                    'slug' => uniqid(),
                    'status' => 1,
                    'creator' => 0, // Seeder doesn't have authenticated user
                    'created_at' => now(),
                    'updated_at' => now(),
            ...
            foreach ($appModules as $module) {
                ...
                $this->command->info("  ✓ Created app_module: {$module['title']} (ID: {$appModule->id})");
            }
            ...
            foreach ($allSubModules as $subModule) {
                ...
                $this->command->info("  ✓ Created app_module_sub_module: {$subModule['title']} (ID: {$appModuleSubModule->id})");
            }
            ...
            if (!empty($allEndpoints)) {
                ...
                $this->command->info("  ✓ Created " . count($allEndpoints) . " app_module_sub_module_endpoint(s)");
            }

            $this->command->newLine();
            $this->command->info('✓ Management routes registration completed successfully!');
            $this->command->info("  - App Modules: " . count($appModules));
            $this->command->info("  - App Module Sub Modules: " . count($allSubModules));
            $this->command->info("  - App Module Sub Module Endpoints: " . count($allEndpoints));

        } catch (\Exception $e) {
            $this->command->error('An error occurred while registering management routes:');
